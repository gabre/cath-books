...
object PpekBook {
  ...
  def toBookPage(link: Element): Either[Throwable, PpekBookPage] = for {
    href <- link.attrs.get("href")
      .toRight(new RuntimeException(s"No href for $link"))
  ...
  def toBook(bookPage: PpekBookPage): Either[Throwable, PpekBook] = {
    ...
    val description = body.childNodes.collectFirst { case t: TextNode =>
      t.content
    }
    val tableRows = body >> elementList("tr")
    val fileUrls = tableRows.flatMap(row =>
      (row >> elementList("a")).map(_.attrs.get("href"))
  ...
  def getPpekBooks(): EitherT[List, Throwable, PpekBookPage] = EitherT {
